    ...
    function renderEditor(container, state) {
                ...
                <div class="section-title" style="display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:0.5rem; margin-bottom:0.75rem;">
                    ...
                    <div style="display:flex; gap:0.5rem; flex-wrap:wrap;">
                        <button class="btn btn-sm btn-primary" id="btn-new-doc" style="gap:0.3rem; font-weight:700;">+ Novo Documento</button>
                            <button class="btn btn-sm btn-green" id="btn-save-repo" title="Salvar no repositório">Salvar no Repositório</button>
        ...
        area.addEventListener('input', () => {
            ...
            timeout = setTimeout(() => {
                ...
                // Atualiza repositório em memória (não força upload remoto automático)
                try {
                    if (state) {
                        if (!state.repository) state.repository = [];
                        const repoIndex = state.repository.findIndex(d => d.id === current.id);
                        const textOnly = (area.innerText || '').trim();
                        const repoDoc = {
                            id: current.id,
                            name: current.title || ('Doc ' + current.id),
                            title: current.title || ('Doc ' + current.id),
                            summary: textOnly.slice(0, 800),
                            text: area.innerHTML,
                            author: (state.users && state.users[state.current_user] && state.users[state.current_user].name) || state.current_user || 'Desconhecido',
                            year: new Date().getFullYear(),
                            topic: 'Geral',
                            document_type: 'Documento',
                            visibility: 'private',
                            uploaded_at: new Date().toISOString(),
                            size_kb: Math.round((area.innerHTML || '').length / 1024)
                        };
                        if (repoIndex >= 0) state.repository[repoIndex] = { ...state.repository[repoIndex], ...repoDoc };
                        else state.repository.unshift(repoDoc);
                        // Persist local and attempt async save (non-blocking)
                        try { NebulaStorage.saveState(state); NebulaStorage.saveStateAsync(state).catch(()=>{}); } catch(e) {}
                    }
                } catch (err) {}
        ...
        // Salvar explicitamente no repositório (permite revisar antes se necessário)
        document.getElementById('btn-save-repo')?.addEventListener('click', async () => {
            ...
            try {
                ...
                // Feedback ao usuário
                if (window.PageProfile && window.PageProfile.showToast) window.PageProfile.showToast('Repositório', 'Documento salvo no repositório com sucesso.');
                document.getElementById('editor-status').innerText = 'Salvo no repositório.';
            } catch (err) {
                console.warn('[Editor] save to repo failed:', err);
                if (window.PageProfile && window.PageProfile.showToast) window.PageProfile.showToast('Erro', 'Não foi possível salvar no repositório.');
            }
        });
