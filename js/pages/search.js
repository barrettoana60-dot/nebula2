...
const PageSearch = (() => {
    ...
    // Backwards-compatible merge: include state.workspaces if available
    function getMergedSearchHistory(userEmail) {
        const email = (userEmail || '').toLowerCase().trim();
        const local = getStoredSearchHistory(email) || [];
        try {
            const state = (window.NebulaApp && NebulaApp.getState) ? NebulaApp.getState() : null;
            const wsHist = (state && state.workspaces && state.workspaces[email] && state.workspaces[email].search_history) || (state && state.search_history) || [];
            const combined = [...local, ...wsHist];
            const seen = new Set();
            const out = [];
            for (const h of combined) {
                const q = (h && h.query) ? h.query.trim().toLowerCase() : '';
    ...
    async function performUserSearch(query, state) {
        ...
        if (query) {
            ...
            const email = (state.current_user || '').toLowerCase().trim();
            const currentHist = getMergedSearchHistory(email);
