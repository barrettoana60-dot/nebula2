...
const PageDashboard = (() => {
    function render(container, state) {
        if (state.search_query) {
            PageSearch.render(container, state);
            return;
        }
        try {
    ...
    function _render(container, state) {
        ...
        let html = `
            ...
            <div class="page-sub mobile-visible-sub">${research ? `Tema da Pesquisa: <b>${research}</b>` : 'Configure sua área de pesquisa no Perfil para receber recomendações focadas.'}</div>
                ...
                ${connections.slice(0, 5).map(conn => {
                    ...
                    return `
                        <div class="doc-card">
                            ...
                            <span class="small-muted">${conn.topic || 'Pesquisa Geral'} · Conexão ${conn.similarity}%</span>
                            <div style="margin-top:0.45rem;color:var(--text-white-80);font-size:0.8rem">${(conn.research || 'Sem linha de pesquisa cadastrada.').slice(0, 140)}</div>
                            <div style="margin-top:0.45rem"><span class="tag">${shared}</span></div>
                        </div>
                    `;
                }).join('')}
                ${connections.length ? `<button class="btn btn-full mt-1" onclick="NebulaApp.navigate('Mensagens')">Ver mensagens</button>` : ''}
            </div>
        `;
        ...
        // Ativar animação de sumiço do banner após 5 segundos
        setTimeout(() => {
            ...
            if (banner) {
                ...
                }, 800); // tempo para concluir a animação CSS
            }
        }, 5000);
